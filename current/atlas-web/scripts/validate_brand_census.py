...
allowed={'published','cataloged','needs_research','context_only','duplicate','out_of_scope'}
for i,row in enumerate(rows,2):
 ...
 if row['decision'] in {'published','cataloged'} and not row['entity_id']: errors.append(f'row {i}: resolved record without entity_id')
...
for i,row in enumerate(rows,2):
 ...
 if not entity: errors.append(f'row {i}: resolved entity_id absent from SQLite brand entities')
