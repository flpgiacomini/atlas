...
path=ROOT/'data'/'historical-significance.candidates.csv';rows=list(csv.DictReader(path.open(encoding='utf-8-sig')));errors=[]
ledger_path=ROOT/'data'/'historical-significance.validation.json'
allowed_kinds={'concept','prototype','prototype_program','one_off'};allowed_decisions={'published','cataloged','include_candidate','context_only','hold','exclude'};tracks={'TEC','IND','DES','SEG','ESP','SOC','GEO','GEN'}
...
for line,row in enumerate(rows,2):
 ...
 if row['decision'] in {'published','cataloged'} and not (row.get('entity_id') or '').strip(): errors.append(f'row {line}: resolved candidate without entity_id')
...
db.close()
for line,row in enumerate(rows,2):
 if row['decision'] not in {'published','cataloged'}: continue
 ...
 elif entity[1]!='vehicle': errors.append(f'row {line}: resolved candidate must resolve to vehicle, got {entity[1]}')
