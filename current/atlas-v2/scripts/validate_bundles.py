...
def validate(root: Path) -> dict:
    ...
    if annual["count"] < 150 or len(annual_years) != len(set(annual_years)):
        raise ValueError("annual chapters must be unique and cover the precursor anchors")
    if not set(range(1769, 1919)).issubset(annual_years):
        raise ValueError("annual chapters must fully cover 1769-1918")
